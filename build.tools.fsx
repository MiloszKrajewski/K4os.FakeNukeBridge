...
module Path =
    ...
    let filenameOf (path: string) = Path.GetFileName(string path)
    let fullnameOf (path: string) = Path.GetFullPath(string path)
    let dirnameOf (path: string) = Path.GetDirectoryName(string path)
